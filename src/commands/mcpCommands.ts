...
export function showExecutionHistoryCommand(executor: ToolExecutor): Command {
  return {
    ...
    callback: () => {
      ...
      const historyText = history.map((entry: ExecutionHistoryEntry, index: number) => {
...
export function getMCPCommands(executor: ToolExecutor): Command[] {
  return [
    stopExecutionsCommand(executor),
    showExecutionHistoryCommand(executor),
    resetSessionLimitsCommand(executor)
  ];
}
