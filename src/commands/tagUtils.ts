...
// Check if line break is needed before, if range start is not at beginning, don't handle
export const insertMarkToBegin = (editor: Editor, range: EditorRange, mark: string) => {
	...
	if (from.line > 0 && editor.getLine(from.line - 1).trim().length > 0 && from.ch === 0) {
		// Previous line is not empty, and from is at the beginning of a line, add a blank line
		insertText = '\n' + mark
		toLine += 1
	} else {
		insertText = mark
...
const getTagMeta = (app: App, editor: Editor, range: EditorRange, settings: PluginSettings): TagMeta => {
	...
	if (firstTag) {
		...
		if (isNewChat) {
			...
			if (secondTag) {
				// newChat situation, return the second tag's role
				return {
					tagContent: secondTag.tag.slice(1),
					role: getTagRole(secondTag, { userTags, assistantTags, systemTags, newChatTags }),
					range: { from: editor.offsetToPos(firstTag.position.end.offset + 1), to: range.to }, // From the end of the first tag to the end of the range
					tagRange: {
