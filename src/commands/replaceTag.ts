...
const convertToTag = (speaker: string) => {
	...
	return '#' + speaker.trim().replace(/\s(?!:)/g, '-') // Replace middle spaces with hyphens
}
...
const findTwoMostFrequentSpeakers = (fileText: string) => {
	// [\u4e00-\u9fa5a-zA-Z0-9]: This character set matches any single Chinese character (range from \u4e00 to \u9fa5, covering most common Chinese characters), English characters (case sensitive) or numbers (0-9)
	...
	// Select the two most frequent ones
