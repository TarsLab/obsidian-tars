...
import type { MCPServerConfig, ServerHealthStatus } from "./types";
...
export class MCPServerManager extends EventEmitter<MCPServerManagerEvents> {
		...
		for (const config of configs) {
			if (config.enabled) {
				try {
					...
					logError(`Failed to start server ${config.name}`, error);
	...
	async startServer(serverId: string): Promise<void> {
		...
		try {
			...
			if (config.deploymentType === "managed") {
				...
				// For SSE transport, we need to pre-create and start the container
				if (config.transport === "sse") {
					await this.startManagedServer(config);
				}
				// For stdio: container will be spawned via 'docker run -i' by the transport
			} else if (config.deploymentType === "external") {
				await this.startExternalServer(config);
			} else {
				throw new Error(
					`Unsupported deployment type: ${config.deploymentType}`,
				);
			}
	...
	private async startManagedServer(config: MCPServerConfig): Promise<void> {
		...
		try {
			...
			const containerId =
				await this.dockerClient.createContainer(containerConfig);
	...
	private async startExternalServer(config: MCPServerConfig): Promise<void> {
		...
		if (!config.sseConfig?.url) {
			throw new Error(
				`External server ${config.name} missing SSE URL configuration`,
			);
		}
		...
		try {
			...
			if (config.deploymentType === "managed" && config.dockerConfig) {
				try {
					...
					logWarning(`Error stopping container for ${config.name}`, error);
				}
			}
