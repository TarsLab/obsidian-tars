...
export class ExecutionLimitError extends MCPError {
  constructor(limitType: 'concurrent' | 'session', current: number, limit: number) {
    super(`${limitType.charAt(0).toUpperCase() + limitType.slice(1)} limit reached: ${current}/${limit}`, 'EXECUTION_LIMIT_ERROR', { limitType, current, limit });
  }
}
...
export class ToolExecutionError extends MCPError {
  constructor(toolName: string, serverName: string, originalError: Error) {
    super(`Tool '${toolName}' execution failed on server '${serverName}': ${originalError.message}`, 'TOOL_EXECUTION_ERROR', {
      toolName,
      serverName,
