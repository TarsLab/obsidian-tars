...
- [x] Add toggle button for each server config — **Test**: `tests/mcp/displayModeToggle.test.ts` (mode normalization)
- [x] Display URL→command conversion (npx -y mcp-remote <url>) — **Test**: `tests/mcp/displayModeToggle.test.ts` (conversion round-trip)
- [x] Keep simple and advanced views in sync — **Test**: `tests/mcp/displayModeToggle.test.ts` (command/url parity)
- [x] Persist user's view preference — **Test**: `tests/mcp/displayModeToggle.test.ts` (display mode normalization); **Manual**: toggle persists via `server.displayMode`
- [x] Add live conversion preview — **Test**: `tests/mcp/displayModeToggle.test.ts` (remoteUrlToCommand preview logic)
...
- [ ] Create src/settings/ directory structure
- [ ] Extract MCP server UI to MCPServerSettings.ts
- [ ] Keep each component <300 lines
- [ ] Preserve all existing functionality
- [ ] Use vanilla Obsidian API (no React yet)
...
**Completed**: 50 SP (Epic-100 Complete ✅, Epic-200 Complete ✅)
**Remaining**: 111 SP
**Estimated Timeline**: 8-10 weeks (100% of Phase 1 complete, Phase 2: 38% complete)
