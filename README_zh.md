...
列表之外的一律走 **Custom（自定义）**，它排在服务商列表第一个：挑一个该地址使用的协议（OpenAI / Claude / Gemini），再填 baseURL、API 密钥和模型。中转站、自建网关、上个版本之后才出现的服务商，都不用等插件发版——协议本来就已经实现了。见[列表里没有的服务商怎么加](#列表里没有的服务商怎么加)。
...
添加一个 **Custom** 服务商，填这几项：

| 设置项  | 填什么                                         |
| ------- | ---------------------------------------------- |
| 标签    | 你用来触发它的标签——同时也是列表里显示的名字   |
| 协议    | OpenAI / Claude / Gemini，按服务商文档写的那个 |
| 模型    | 地址能列出模型就从列表里选，列不出来就直接填   |
| baseURL | 从文档复制，检查网址是否完整                   |
| API key | 服务商给的密钥                                 |

其余的由协议决定：模型列表去该协议对应的接口取，Claude 会带上自己的 max_tokens 与思考设置，网络搜索只在协议支持时出现。之后再改协议，baseURL 会跟着走——除非你已经填过自己的地址。
